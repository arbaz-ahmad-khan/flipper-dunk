...
class TwoLevel extends Phaser.Scene {
	...
	editorCreate() {
		...
		// text_1
		const text_1 = this.add.text(540, 80, "", {});
		...
		// rectangle_3
		const rectangle_3 = this.add.rectangle(100, 735, 75, 75);
		...
		// obstaclesContainer
		const obstaclesContainer = this.add.container(0, 0);
		...
		// rectangle_33
		const rectangle_33 = this.add.rectangle(194, 704, 50, 200);
		rectangle_33.visible = false;
		rectangle_33.isFilled = true;
		obstaclesContainer.add(rectangle_33);

		// rectangle_34
		const rectangle_34 = this.add.rectangle(114, 830, 200, 50);
	...
	create() {
		...
		// back-net
		const net1 = this.add.image(100, 640, "backward-Net");
		net1.scaleX = 0.7;
		net1.scaleY = 0.7;
		...
		// front-net
		const net_1 = this.add.image(100, 640, "Net-1");
		net_1.scaleX = 0.7;
		net_1.scaleY = 0.7;
	...
	handleBallOverlapGoal() {
		...
		this.scene.stop("TwoLevel");
		this.scene.start("ThreeLevel");
	}
	...
	flip(isDown) {
		this.tweens.add({
			...
			onUpdateScope: this,
			onUpdate: function () {
				this.lever.setPosition(X - Math.cos(this.tweener.x) * LEVER, Y - Math.sin(this.tweener.x) * LEVER);
			},
		});
	}
